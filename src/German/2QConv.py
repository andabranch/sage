...
OUTPUT_PATH = 'output/extra_architectures/2QConv/'
...
def build_model(filter1, kernel_size1, filter2, kernel_size2, neurons_dense1, use_batchnormalization, use_maxpooling):
    # Building the model
...
def QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_43(X_train, X_validation, y_train, y_validation, X_test, y_test):
    TEST_NAME = str(SIZE) + 'x' + str(SIZE) + '/3_' + str(SIZE) + '_' + str(SIZE) + \
        '_QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_43_ep_' + \
        str(EPOCHS)

    start = datetime.now()
    model = build_model(FILTER_32, KERNEL_SIZE_5, FILTER_64, KERNEL_SIZE_3,
                        NO_DENSE, USE_BN, USE_MP)
...
def QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_256_BN_Dense_43(X_train, X_validation, y_train, y_validation, X_test, y_test):
    TEST_NAME = str(SIZE) + 'x' + str(SIZE) + '/3_' + str(SIZE) + '_' + str(SIZE) + \
        '_QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_256_BN_Dense_43_ep_' + \
        str(EPOCHS)

    start = datetime.now()
    model = build_model(FILTER_32, KERNEL_SIZE_5, FILTER_64, KERNEL_SIZE_5,
                        DENSE_256, USE_BN, USE_MP)
    post_build_process(model, X_train, X_validation, y_train,
                       y_validation, X_test, y_test, TEST_NAME)
    end = datetime.now()
    get_n_save_execution_time(start, end, TEST_NAME)
...
def QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_128_BN_Dense_43(X_train, X_validation, y_train, y_validation, X_test, y_test):
    TEST_NAME = str(SIZE) + 'x' + str(SIZE) + '/3_' + str(SIZE) + '_' + str(SIZE) + \
        '_QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_128_BN_Dense_43_ep_' + \
        str(EPOCHS)

    start = datetime.now()
    model = build_model(FILTER_32, KERNEL_SIZE_5, FILTER_64, KERNEL_SIZE_5,
                        DENSE_128, USE_BN, USE_MP)
    post_build_process(model, X_train, X_validation, y_train,
                       y_validation, X_test, y_test, TEST_NAME)
    end = datetime.now()
    get_n_save_execution_time(start, end, TEST_NAME)
...
def QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_512_BN_Dense_43(X_train, X_validation, y_train, y_validation, X_test, y_test):
    TEST_NAME = str(SIZE) + 'x' + str(SIZE) + '/3_' + str(SIZE) + '_' + str(SIZE) + \
        '_QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_512_BN_Dense_43_ep_' + \
        str(EPOCHS)

    start = datetime.now()
    model = build_model(FILTER_32, KERNEL_SIZE_5, FILTER_64, KERNEL_SIZE_5,
                        DENSE_512, USE_BN, USE_MP)
    post_build_process(model, X_train, X_validation, y_train,
                       y_validation, X_test, y_test, TEST_NAME)
    end = datetime.now()
    get_n_save_execution_time(start, end, TEST_NAME)
...
def QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_1024_BN_Dense_43(X_train, X_validation, y_train, y_validation, X_test, y_test):
    TEST_NAME = str(SIZE) + 'x' + str(SIZE) + '/3_' + str(SIZE) + '_' + str(SIZE) + \
        '_QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_1024_BN_Dense_43_ep_' + \
        str(EPOCHS)

    start = datetime.now()
    model = build_model(FILTER_32, KERNEL_SIZE_5, FILTER_64, KERNEL_SIZE_5,
                        DENSE_1024, USE_BN, USE_MP)
    post_build_process(model, X_train, X_validation, y_train,
                       y_validation, X_test, y_test, TEST_NAME)
    end = datetime.now()
    get_n_save_execution_time(start, end, TEST_NAME)
...
def QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_64_BN_Dense_43(X_train, X_validation, y_train, y_validation, X_test, y_test):
    TEST_NAME = str(SIZE) + 'x' + str(SIZE) + '/3_' + str(SIZE) + '_' + str(SIZE) + \
        '_QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_64_BN_Dense_43_ep_' + \
        str(EPOCHS)

    start = datetime.now()
    model = build_model(FILTER_32, KERNEL_SIZE_5, FILTER_64, KERNEL_SIZE_5,
                        DENSE_64, USE_BN, USE_MP)
    post_build_process(model, X_train, X_validation, y_train,
                       y_validation, X_test, y_test, TEST_NAME)
    end = datetime.now()
    get_n_save_execution_time(start, end, TEST_NAME)
...
if __name__ == "__main__":
    ...
    QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_43(
        X_train, X_validation, y_train, y_validation, X_test, y_test)

    # QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_64_BN_Dense_43(
    #     X_train, X_validation, y_train, y_validation, X_test, y_test)

    # QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_128_BN_Dense_43(
    #     X_train, X_validation, y_train, y_validation, X_test, y_test)

    # QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_256_BN_Dense_43(
    #     X_train, X_validation, y_train, y_validation, X_test, y_test)

    # QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_512_BN_Dense_43(
    #     X_train, X_validation, y_train, y_validation, X_test, y_test)

    # QConv_32_5_MP_2_BN_QConv_64_5_MP_2_BN_Dense_1024_BN_Dense_43(
    #     X_train, X_validation, y_train, y_validation, X_test, y_test)
