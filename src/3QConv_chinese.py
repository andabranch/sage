...
def get_training_dataset():
    ...
    # Read the annotations CSV file for training
    annotations_file = os.path.join(CHINESE_DATASET_PATH, 'TrainAnnotation.txt')
    annotations_df = pd.read_csv(annotations_file, delimiter=';')

    for index, row in annotations_df.iterrows():
        try:
            image_path = os.path.join(
                CHINESE_DATASET_PATH, 'Train', row['file_name'])
    ...
    print(annotations_df[annotations_df['category'].isna()])
...
def get_testing_dataset():
    ...
    # Read the annotations CSV file for testing
...
def generate_and_save_confusion_matrix(model, X_test, y_test, test_name, dataset_name):
    ...
    # Check if y_test is one-hot encoded, and convert it to class labels if it is
    if y_test.ndim > 1 and y_test.shape[1] > 1:
        y_true_labels = np.argmax(y_test, axis=1)
    else:
        y_true_labels = y_test
